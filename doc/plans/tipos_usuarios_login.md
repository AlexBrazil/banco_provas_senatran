...
- `banco_questoes/templates/simulado/config.html`: exibir limite/contagem e CTA de upgrade quando bloqueado (apenas se plano atual for Free, oferecendo "Aprova DETRAN" via PIX).
...
- Entregas: mensagens e CTAs de bloqueio/upgrade (Free -> "Aprova DETRAN" via PIX); exibir limites/contagem em telas de inicio/config/erro.
